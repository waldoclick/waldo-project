...
export class AdService {
  ...
  async decrementRemainingDays(): Promise<ICronjobResult> {
    try {
      ...
      for (const ad of ads) {
        ...
        const existingRemaining = await strapi.db
          .query("api::remaining.remaining")
          .findMany({
            where: {
              ad: { id: ad.id },
              createdAt: {
        ...
        if (existingRemaining.length === 0) {
          ...
          await strapi.db.query("api::remaining.remaining").create({
            data: {
              ad: ad.id,
            },
          });
  ...
  private async sendUpdatedAdsReport(): Promise<void> {
    try {
      ...
      const remainings = await strapi.db
        .query("api::remaining.remaining")
        .findMany({
          where: {
            createdAt: {
