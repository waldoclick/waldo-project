...
export default {
  async find(ctx) {
    ...
    const securityPolicies = await strapi.db
  ...
  async findOne(ctx) {
    ...
    const securityPolicy = await strapi
      .documents("api::security-policy.security-policy")
      .findOne({ documentId });
  ...
  async create(ctx) {
    ...
    const securityPolicy = await strapi.db
      .query("api::security-policy.security-policy")
      .create({ data });
    return { data: securityPolicy };
  },
  ...
  async reorder(ctx) {
    ...
    await Promise.all(
      data.map((entry) =>
        strapi.documents("api::security-policy.security-policy").update({
