...
const populateSecurityPolicies = async (strapi: Core.Strapi): Promise<void> => {
  ...
  if (existing.length > 0) {
    await strapi.db
      ...
      .deleteMany({ where: {} });
    console.log(
      `Eliminados ${existing.length} registros anteriores de Política de Seguridad`,
    );
  }
    ...
    try {
      await strapi.db.query("api::security-policy.security-policy").create({
        data: {
          ...
          text: securityPolicy.text,
          order: securityPolicy.order,
        },
      });
      console.log(`Política de Seguridad creada: ${securityPolicy.title}`);
    } catch (securityPolicyError) {
      console.error(
        `Error creando Política de Seguridad ${securityPolicy.title}:`,
        securityPolicyError.message,
      );
    }
  }

  console.log(
    `Política de Seguridad poblada: ${securityPoliciesData.length} secciones`,
  );
};
