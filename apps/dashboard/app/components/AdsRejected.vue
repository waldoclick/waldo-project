<template>
  <section class="ads ads--rejected">
    <div class="ads--rejected__container">
      <div class="ads--rejected__header">
        <SearchDefault
          :model-value="settingsStore.adsRejected.searchTerm"
          placeholder="Buscar anuncios..."
          class="ads--rejected__search"
          @update:model-value="
        ...
        <FilterDefault
          ...
          class="ads--rejected__filters"
          @update:model-value="handleFiltersChange"
        />
      </div>

      <div class="ads--rejected__table-wrapper">
        <TableDefault :columns="tableColumns">
          <TableRow v-for="ad in paginatedAds" :key="ad.id">
            <TableCell>
              <div class="ads--rejected__gallery">
                <img
                  v-if="ad.gallery && ad.gallery.length > 0 && ad.gallery[0]"
                  :src="getImageUrl(ad.gallery[0])"
                  :alt="ad.name"
                  class="ads--rejected__gallery__image"
                />
                <span v-else class="ads--rejected__gallery__placeholder"
                  >-</span
                >
              </div>
            </TableCell>
            <TableCell>
              <div class="ads--rejected__name">{{ ad.name }}</div>
            </TableCell>
            <TableCell>
              <div class="ads--rejected__user">
                {{ ad.user?.username || "-" }}
              </div>
            </TableCell>
            ...
            <TableCell align="right">
              <button
                class="ads--rejected__action"
                title="Ver anuncio"
                @click="handleViewAd(ad.id)"
              >
                <Eye class="ads--rejected__action__icon" />
              </button>
            </TableCell>
          </TableRow>
        </TableDefault>
        ...
        <div v-if="loading" class="ads--rejected__loading">
          <p>Cargando anuncios...</p>
        </div>
      </div>

      <PaginationDefault
        :current-page="settingsStore.adsRejected.currentPage"
        :total-pages="totalPages"
        :total-records="totalRecords"
        :page-size="settingsStore.adsRejected.pageSize"
...
const section = "adsRejected" as const;

// Computed para los filtros de anuncios
const filters = computed(() => settingsStore.getAdsRejectedFilters);
...
const fetchRejectedAds = async () => {
  try {
    ...
    const searchParams: any = {
      pagination: {
        page: settingsStore.adsRejected.currentPage,
        pageSize: settingsStore.adsRejected.pageSize,
      },
      sort: settingsStore.adsRejected.sortBy,
      populate: {
        user: {
          fields: ["username"],
        },
    ...
    if (settingsStore.adsRejected.searchTerm) {
      searchParams.filters = {
        $or: [
          { name: { $containsi: settingsStore.adsRejected.searchTerm } },
          { description: { $containsi: settingsStore.adsRejected.searchTerm } },
          {
            "user.username": {
              $containsi: settingsStore.adsRejected.searchTerm,
            },
          },
          {
            "user.email": { $containsi: settingsStore.adsRejected.searchTerm },
          },
        ],
      };
    }
...
watch(
  [
    () => settingsStore.adsRejected.searchTerm,
    () => settingsStore.adsRejected.sortBy,
    () => settingsStore.adsRejected.pageSize,
    () => settingsStore.adsRejected.currentPage,
  ],
  () => {
    fetchRejectedAds();
  },
