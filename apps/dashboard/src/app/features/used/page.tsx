...
export default function UsedFeaturedPage() {
  const { usedFeatures: usedFeaturesPrefs, setUsedFeaturesPreferences } =
    usePreferencesStore();
  const [usedFeatured, setUsedFeatured] = useState<
    StrapiAdFeaturedReservation[]
  >([]);
  ...
  useEffect(() => {
    if (!isInitialized) {
      setSearchTerm(usedFeaturesPrefs.searchTerm);
      setPageSize(usedFeaturesPrefs.pageSize);
      setSortBy(usedFeaturesPrefs.sortBy);
      setIsInitialized(true);
    }
  }, [usedFeaturesPrefs, isInitialized]);
  ...
  }, [searchTerm, pageSize, sortBy, setUsedFeaturesPreferences, isInitialized]);
  ...
  useEffect(() => {
    ...
    const fetchUsedFeatured = async () => {
      try {
        setLoading(true);
        const response = await getUsedFeaturedReservations({
          page: currentPage,
          pageSize: pageSize,
          sort: sortBy,
          search: searchTerm || undefined,
        });
        setUsedFeatured(response.data);
