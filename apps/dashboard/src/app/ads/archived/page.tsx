...
import { getArchivedAds, StrapiAd } from '@/lib/strapi';
...
export default function ArchivedAdsPage() {
  const { archivedAds: archivedAdsPrefs, setArchivedAdsPreferences } =
    usePreferencesStore();
  ...
  useEffect(() => {
    if (!isInitialized) {
      setSearchTerm(archivedAdsPrefs.searchTerm);
      setPageSize(archivedAdsPrefs.pageSize);
      setSortBy(archivedAdsPrefs.sortBy);
      setIsInitialized(true);
    }
  }, [archivedAdsPrefs, isInitialized]);
  ...
  }, [searchTerm, pageSize, sortBy, setArchivedAdsPreferences, isInitialized]);
  ...
  useEffect(() => {
    ...
    const fetchAds = async () => {
      try {
        setLoading(true);
        const response = await getArchivedAds({
          page: currentPage,
          pageSize: pageSize,
          sort: sortBy,
