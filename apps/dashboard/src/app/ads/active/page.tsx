...
export default function ActiveAdsPage() {
  const { activeAds: activeAdsPrefs, setActiveAdsPreferences } =
    usePreferencesStore();
  ...
  useEffect(() => {
    if (!isInitialized) {
      setSearchTerm(activeAdsPrefs.searchTerm);
      setPageSize(activeAdsPrefs.pageSize);
      setSortBy(activeAdsPrefs.sortBy);
      setIsInitialized(true);
    }
  }, [activeAdsPrefs, isInitialized]);

  // Guardar preferencias cuando cambien (solo después de la inicialización)
  useEffect(() => {
    if (isInitialized) {
      setActiveAdsPreferences({
        searchTerm,
        pageSize,
        sortBy,
      });
    }
  }, [searchTerm, pageSize, sortBy, setActiveAdsPreferences, isInitialized]);
