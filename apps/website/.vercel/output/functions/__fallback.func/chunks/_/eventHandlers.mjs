...
try {
  ...
  n &&
    ...
    (f._sentryDebugIds[n] = "e87be4ce-dfce-41d4-a99f-2a6da520de59"),
    (f._sentryDebugIdIdentifier =
      "sentry-dbid-e87be4ce-dfce-41d4-a99f-2a6da520de59"));
} catch (f) {}
import {
  H as n,
  I as e,
...
import { createUnhead as y } from "unhead";
import { renderSSRHead as C } from "unhead/server";
const E = n(e(), "/assets");
async function loadFont({ e: f, publicStoragePath: n }, e) {
  const { name: d, weight: a } = e;
  if (e.data) return e;
  if (e.key && (await E.hasItem(e.key))) {
    let f = await E.getItem(e.key);
    if (f instanceof Uint8Array) {
      f = new TextDecoder().decode(f);
    }
    return ((e.data = Buffer.from(String(f), "base64")), e);
  }
  let t;
  return (
    (t = e.path
      ? await f.$fetch(e.path, { baseURL: i(f), responseType: "arrayBuffer" })
      : await f.$fetch(`/__og-image__/font/${d}/${a}.ttf`, {
...
async function applyEmojis(f, n) {
  const e = n.html,
    i = e.match(v);
  if (!i) return e;
  const a = await Promise.all(
      i.map(async (n) => {
        const e = n.codePointAt(0).toString(16),
          i = B[e];
        if (i) {
          const e = ["1", f.options.emojis, i].join(":");
          let a;
          return (
            (await d.hasItem(e)) && (a = await d.getItem(e)),
            a ||
              ((a = await $fetch(
                `https://api.iconify.design/${f.options.emojis}/${i}.svg`,
                { responseType: "text", retry: 3, retryDelay: 1e3 },
              )),
              "404" === a && (a = void 0),
              a && (await d.setItem(e, a))),
            a
              ? `\n${a.replace("<svg ", '<svg data-emoji style="margin: 0 .05em 0 .15em; vertical-align: -0.1em;" ')}\n`
              : n
          );
        }
      }),
    ),
    t = e.replace(v, () => a.shift()),
    o = t !== n.html;
  return ((n.html = t), o);
}
const A = y;
async function fontEventHandler(f) {
  const n = c(f.path).pathname,
    { fonts: e } = a(),
    i = n.split("/font/")[1];
  if (i && i.includes(":")) {
    const n = e.find((f) => f.key === i);
    if (n?.key && (await E.hasItem(n.key))) {
      let e = await E.getItem(n.key);
      if (e instanceof Uint8Array) {
        e = new TextDecoder().decode(e);
      }
      return (
        i.includes(".oft")
          ? D(f, "Content-Type", "font/otf")
          : i.includes(".woff2")
            ? D(f, "Content-Type", "font/woff2")
            : i.includes(".ttf") && D(f, "Content-Type", "font/ttf"),
        Buffer.from(e, "base64")
      );
    }
  }
  const [d, o] = String(i?.split(".")[0]).split("/");
  if (!d || !o) return "Provide a font name and weight";
  const s = String(d[0]).toUpperCase() + d.slice(1),
    u = 100 * Math.round(Number.parseInt(o) / 100),
    k = a(),
    l = m(k.fonts);
  let g;
  if (
    ("string" == typeof r(f).font && (g = JSON.parse(r(f).font)),
    g ||
      (g = l.find(
        (f) =>
          f.name.toLowerCase() === s.toLowerCase() && u === Number(f.weight),
      )),
    !g)
  )
    return t({
      statusCode: 404,
      statusMessage: `[Nuxt OG Image] Font ${s}:${u} not found`,
    });
  const h = await globalThis.$fetch(
    `https://fonts.googleapis.com/css2?family=${s}:wght@${u}`,
    {
      headers: {
        "User-Agent":
  ...
  if (!h)
    return t({
      statusCode: 500,
      statusMessage: `[Nuxt OG Image] Invalid Google Font ${s}:${u}`,
    });
  const p = h.match(/src: url\((.+)\) format\('(opentype|truetype)'\)/);
  if (p?.[1]) return b(f, p[1], {});
  const F = h.match(/src: url\((.+)\) format\('woff2'\)/);
  return F?.[1]
    ? w(f, F[1])
    : t({
        statusCode: 500,
        statusMessage: `[Nuxt OG Image] Malformed Google Font CSS ${h}`,
      });
}
async function imageEventHandler(f) {
  const i = await p(f);
  if (i instanceof F) return i;
  const { isDebugJsonPayload: d, extension: c, options: D, renderer: b } = i,
    { debug: w, baseCacheKey: y } = a(),
    E = [];
  if (d) {
    const n = r(f).extension || i.options.extension;
    return (
      ["jpeg", "jpg"].includes(n) &&
        "satori" === D.renderer &&
        E.push(
      ...
      {
        siteConfig: { url: f.context.siteConfig.get().url },
        compatibilityHints: E,
        cacheKey: i.key,
        options: i.options,
        ...("satori" === D.renderer ? await b.debug(i) : void 0),
      }
    );
  }
  switch (c) {
      ...
      return (
        ...
        (async function (f) {
          ...
          const i = await o(
              f.e,
              f.options.component,
              void 0 !== f.options.props ? f.options.props : f.options,
            ),
            d = A();
          d.push(i.head);
          let u = "sans-serif";
          const r = m([...(n.fonts || []), ...e])[0];
          (r && (u = r.name.replaceAll("+", " ")), await applyEmojis(f, i));
          let k = i.html;
          (d.push({
            style: [
              { innerHTML: `body { font-family: '${u}', sans-serif;  }` },
              {
                innerHTML: `body {\n    transform: scale(${n.props?.scale || 1});\n    transform-origin: top left;\n    max-height: 100vh;\n    position: relative;\n    width: ${n.width}px;\n    height: ${n.height}px;\n    overflow: hidden;\n    background-color: ${"dark" === n.props?.colorMode ? "#1b1b1b" : "#fff"};\n}\ndiv {\n  display: flex;\n}\ndiv:has(div, p, ul, ol, li, blockquote, pre, hr, table, dl) {\n  display: flex;\n  flex-direction: column;\n}\ndiv:not(:has(div, p, ul, ol, li, blockquote, pre, hr, table, dl)) {\n  display: flex;\n  flex-wrap: wrap;\n  gap: 12px;\n}\n\nsvg[data-emoji] {\n  display: inline-block;\n}\n`,
              },
              ...e.map(
                (f) =>
                  `\n          @font-face {\n            font-family: '${f.name.replaceAll("+", " ")}';\n            font-style: normal;\n            font-weight: ${f.weight};\n            src: url('/__og-image__/font/${f.key}') format('truetype');\n          }\n          `,
              ),
            ],
            meta: [{ charset: "utf-8" }],
            script: [
          ...
          const l = await C(d);
          return `<!DOCTYPE html>\n<html ${l.htmlAttrs}>\n<head>${l.headTags}</head>\n<body ${l.bodyAttrs}>${l.bodyTagsOpen}<div data-v-inspector-ignore="true" style="position: relative; display: flex; margin: 0 auto; width: ${n.width}px; height: ${n.height}px; overflow: hidden;">${k}</div>${l.bodyTags}</body>\n</html>`;
        })(i)
      );
    case "svg":
      return w
        ? "satori" !== i.renderer.name
          ? t({
              statusCode: 400,
              statusMessage: `[Nuxt OG Image] Generating ${c}'s with ${b.name} is not supported.`,
            })
          : (g(f, "Content-Type", "image/svg+xml"),
            (await i.renderer.debug(i)).svg)
        : t({ statusCode: 404 });
    case "png":
    case "jpeg":
    case "jpg":
      if (!b.supportedFormats.includes(c))
        return t({
          statusCode: 400,
          statusMessage: `[Nuxt OG Image] Generating ${c}'s with ${b.name} is not supported.`,
        });
      g(f, "Content-Type", `image/${"jpg" === c ? "jpeg" : c}`);
      break;
    default:
      return t({
        statusCode: 400,
        statusMessage: `[Nuxt OG Image] Invalid request for og.${c}.`,
      });
  }
  const B = await (async function (f, i) {
    const d = Number(i.cacheMaxAgeSeconds);
    let a = "test" !== globalThis._importMeta_.env?.MODE && d > 0;
    const o = n(e(), u(i.baseCacheKey || "/")),
      m = f.key;
    let s = !1;
    if (a) {
      const n = await o
        ...
        .catch(
          (f) => (
            ...
            t({
              cause: f,
              statusCode: 500,
              statusMessage: `[Nuxt OG Image] Failed to connect to cache ${i.baseCacheKey}. Response from cache: ${f.message}`,
            })
          ),
        );
      ...
      if (n) {
        const {
          ...
          .getItem(m)
          .catch(() => ({ value: null, expiresAt: Date.now() }));
        if (void 0 !== r(f.e).purge) await o.removeItem(m).catch(() => {});
        else if (e > Date.now()) {
          if (
            ((s = Buffer.from(n, "base64")),
            k(f.e, {
              modifiedTime: new Date(i["last-modified"]),
              etag: i.etag,
              maxAge: d,
            }))
          )
            return;
          l(f.e, i);
        } else await o.removeItem(m).catch(() => {});
      }
    }
    return (
      a ||
        (g(f.e, "Cache-Control", "no-cache, no-store, must-revalidate"),
        g(f.e, "Pragma", "no-cache"),
        g(f.e, "Expires", "0")),
      {
        enabled: a,
        cachedItem: s,
        async update(n) {
          if (!a) return;
          const e = Buffer.from(n).toString("base64"),
            i = {
              Vary: "accept-encoding, host",
              etag: `W/"${h(e)}"`,
              "last-modified": new Date().toUTCString(),
              "cache-control": `public, s-maxage=${d}, stale-while-revalidate`,
            };
          (l(f.e, i),
            await o.setItem(m, {
              value: e,
              headers: i,
              expiresAt: Date.now() + 1e3 * d,
            }));
        },
      }
    );
  })(i, { cacheMaxAgeSeconds: i.options.cacheMaxAgeSeconds, baseCacheKey: y });
  if (void 0 === B) return;
  if (B instanceof F) return B;
  let v = B.cachedItem;
  if (!v) {
    if (((v = await b.createImage(i)), v instanceof F)) return v;
    if (!v)
      return t({
        statusCode: 500,
        statusMessage: `Failed to generate og.${c}.`,
      });
    await B.update(v);
  }
  return v;
...
export {
  applyEmojis as a,
  fontEventHandler as f,
  imageEventHandler as i,
  loadFont as l,
};
