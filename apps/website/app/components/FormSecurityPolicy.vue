...
const isEditMode = computed(() =>
  Boolean(props.securityPolicy?.documentId || props.securityPolicy?.id),
);
...
const hydrateForm = () => {
  ...
  lastHydratedId.value =
    props.securityPolicy?.id || props.securityPolicy?.documentId || null;
};
...
const handleSubmit = async (values: Record<string, unknown>) => {
  ...
  try {
    ...
    if (isEditMode.value) {
      ...
      const documentId =
        props.securityPolicy?.documentId ||
        (typeof routeId === "string" ? routeId : undefined);

      if (!documentId) {
        await Swal.fire(
          "Error",
          "No se pudo identificar la Política de Seguridad para actualizar.",
          "error",
        );
        sending.value = false;
        return;
      ...
      await Swal.fire(
        "Éxito",
        "Política de Seguridad actualizada correctamente.",
        "success",
      );
      ...
      const response = await apiClient<{
        data: { id?: number; documentId?: string };
      }>("/security-policies", { method: "POST", body: { data: payload } });
      const createdData = response.data;
      emit(
        "saved",
        (createdData as SecurityPolicyData) || ({} as SecurityPolicyData),
      );
      await Swal.fire(
        "Éxito",
        "Política de Seguridad creada correctamente.",
        "success",
      );
    ...
    await Swal.fire(
      "Error",
      "No se pudo guardar la Política de Seguridad.",
      "error",
    );
  } finally {
    sending.value = false;
