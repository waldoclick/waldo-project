...
const isEditMode = computed(() =>
  Boolean(props.cookiePolicy?.documentId || props.cookiePolicy?.id),
);
...
const hydrateForm = () => {
  ...
  lastHydratedId.value =
    props.cookiePolicy?.id || props.cookiePolicy?.documentId || null;
};
...
const handleSubmit = async (values: Record<string, unknown>) => {
  ...
  try {
    ...
    if (isEditMode.value) {
      ...
      const documentId =
        props.cookiePolicy?.documentId ||
        (typeof routeId === "string" ? routeId : undefined);

      if (!documentId) {
        await Swal.fire(
          "Error",
          "No se pudo identificar la Política de Cookies para actualizar.",
          "error",
        );
        sending.value = false;
        return;
      ...
      const response = await apiClient<{
        data: { id?: number; documentId?: string };
      }>("/cookie-policies", { method: "POST", body: { data: payload } });
      const createdData = response.data;
      emit(
        "saved",
        (createdData as CookiePolicyData) || ({} as CookiePolicyData),
      );
      await Swal.fire(
        "Éxito",
        "Política de Cookies creada correctamente.",
        "success",
      );
    ...
    await Swal.fire(
      "Error",
      "No se pudo guardar la Política de Cookies.",
      "error",
    );
  } finally {
    sending.value = false;
