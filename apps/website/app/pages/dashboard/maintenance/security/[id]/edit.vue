<template>
  <div>
    ...
    <BoxContent>
      <template #content>
        <BoxInformation
          title="Editar Sección de Política de Seguridad"
          :columns="1"
        >
          <FormSecurityPolicy
            :security-policy="securityPolicy"
            @saved="handleSecurityPolicySaved"
          />
        </BoxInformation>
      </template>
      <template #sidebar>
        <BoxInformation title="Detalles" :columns="1">
          <CardInfo
            ...
            :description="formatDate(securityPolicy.createdAt)"
          />
          <CardInfo
            v-if="securityPolicy"
            title="Última modificación"
            :description="formatDate(securityPolicy.updatedAt)"
          />
        </BoxInformation>
      </template>
    </BoxContent>
...
const title = computed(
  () => securityPolicy.value?.title || "Política de Seguridad",
);
const breadcrumbs = computed(() => [
  ...
  ...(securityPolicy.value?.title
    ? [
        {
          label: securityPolicy.value.title,
          to: `/dashboard/maintenance/security/${route.params.id}`,
        },
      ]
    : []),
  { label: "Editar" },
