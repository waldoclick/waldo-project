<template>
  <div>
    ...
    <BoxContent>
      <template #content>
        <BoxInformation
          title="Editar Sección de Política de Cookies"
          :columns="1"
        >
          <FormCookiePolicy
            :cookie-policy="cookiePolicy"
            @saved="handleCookiePolicySaved"
          />
        </BoxInformation>
      </template>
      <template #sidebar>
        <BoxInformation title="Detalles" :columns="1">
          <CardInfo
            ...
            :description="formatDate(cookiePolicy.createdAt)"
          />
          <CardInfo
            v-if="cookiePolicy"
            title="Última modificación"
            :description="formatDate(cookiePolicy.updatedAt)"
          />
        </BoxInformation>
      </template>
    </BoxContent>
...
const title = computed(
  () => cookiePolicy.value?.title || "Política de Cookies",
);
const breadcrumbs = computed(() => [
  ...
  ...(cookiePolicy.value?.title
    ? [
        {
          label: cookiePolicy.value.title,
          to: `/dashboard/maintenance/cookies/${route.params.id}`,
        },
      ]
    : []),
  { label: "Editar" },
