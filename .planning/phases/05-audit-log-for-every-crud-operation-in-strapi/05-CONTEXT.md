...
- **This requestContext mechanism does not exist yet in this codebase** — confirmed via grep across `apps/strapi/src`: no `AsyncLocalStorage`, no `requestContext` reference anywhere. It must be built in this phase as a new global Koa middleware (pattern: `apps/strapi/src/middlewares/*.ts`, registered as `global::request-context` in `config/middlewares.ts`), using Node's `AsyncLocalStorage` to store the live `ctx` object (not a snapshot) so that `ctx.state.user`, which gets populated later in the middleware/policy chain, is still visible when a lifecycle hook fires downstream in the same async call.
- Must be registered early in `config/middlewares.ts` (before `strapi::session`, before route policies) so the ALS context wraps the entire request lifecycle, including admin panel (`/content-manager`) and public API (`/api/*`) paths — unlike `proxy-auth`, this middleware should NOT be scoped to `/api` only, since admin-panel writes need tracking too.
- Actor-type discriminator required: Strapi admin-panel users (`admin::user`) and users-permissions users (`plugin::users-permissions.user`) are separate tables with overlapping numeric ids — the audit record must store which table the actor id refers to, not just a bare numeric id.
