...
<!-- apps/website/app/components/FormTerm.vue (full file, 200 lines) — exact pattern for FormCookiePolicy.vue, rename Term->CookiePolicy, term->cookiePolicy, "/terms" endpoint -> "/cookie-policies", route /dashboard/maintenance/terms -> /dashboard/maintenance/cookies, BEM form--term -> form--cookie -->
...
- BEM block name is the single unhyphenated noun `cookies` (not `cookie-policies`) — grep -rn "cookie-policies--" across CookiePoliciesDashboard.vue returns nothing; form modifier is `form--cookie` (not `form--cookie-policy`) — grep -rn "form--cookie-policy" across FormCookiePolicy.vue returns nothing.
