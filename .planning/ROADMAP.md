...
Plans:
- [ ] 127-01-PLAN.md — SEC2-PAYMENT: Webpay amount re-validation + idempotency on unique buy_order + adId ownership + fail-closed featured price (wave 1)
- [ ] 127-02-PLAN.md — SEC2-AUTHZ: order findOne/find ownership + scoping, gate export-csv with isManager, lock down ad-pack/ad-reservation/ad-featured-reservation CRUD (wave 1)
- [ ] 127-03-PLAN.md — SEC2-AUTH: Google email_verified check, remove JWT fallback in ad.ts, two-layer auth rate limit, bind reCAPTCHA action/hostname (wave 1)
- [ ] 127-04-PLAN.md — SEC2-XSS: isomorphic-dompurify SSR sanitizer in useSanitize.ts + disable marked raw HTML (wave 1)
- [ ] 127-05-PLAN.md — SEC2-LOCKDOWN: MJML autoescape, upload magic-byte + sizeLimit, GET /api/users filter whitelist + PII strip, disable content-API routes for verification-code/contact/subscription-payment (wave 2, after 127-03 for shared plugins.ts)
