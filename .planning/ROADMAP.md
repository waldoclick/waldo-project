...
- [x] 120-01-PLAN.md — Create subscription-pro schema + user schema update + bootstrap migration
- [x] 120-02-PLAN.md — Fix charge-before-activate in proResponse + subscription-pro record creation
- [x] 120-03-PLAN.md — Update cron + cancellation service + middleware to use subscription-pro
...
Plans:
- [x] 125-01-PLAN.md — Wave 0 stubs + dashboard-guard + onboarding exemption + FormVerifyCode manager redirect + dashboard layout + stores
- [x] 125-02-PLAN.md — Package deps + nuxt.config (optimizeDeps, routeRules, robots)
- [x] 125-03-PLAN.md — Components migration (rename 28 colliding before copy, move all flat + icons)
- [x] 125-04-PLAN.md — Composables migration + session system replacement (useSessionX → useStrapiX)
...
**Plans:** 4/4 plans complete — completed 2026-06-12
**Status:** Complete — 2026-06-12

Plans:
- [x] 126-01-PLAN.md — SEC-IDOR-USERS (HIGH): ownership policy on built-in `user.update` (cross-user PUT → 403, manager bypass) without breaking the three partial-body self callers
...
Plans:
- [x] 127-01-PLAN.md — SEC2-PAYMENT: Webpay amount re-validation + idempotency on unique buy_order + adId ownership + fail-closed featured price (wave 1)
- [x] 127-02-PLAN.md — SEC2-AUTHZ: order findOne/find ownership + scoping, gate export-csv with isManager, lock down ad-pack/ad-reservation/ad-featured-reservation CRUD (wave 1)
...
Plans:
- [ ] 129-01-PLAN.md — Session composables (useSessionUser/Auth/Client) + inert session.ts plugin + Wave 0 tests (wave 1)
- [ ] 129-02-PLAN.md — Proxy Cookie->Authorization injection + dedicated Nitro auth routes (verify-code, one-tap, logout, OAuth exchange/popup) with reCAPTCHA (wave 1)
- [ ] 129-03-PLAN.md — useApiClient SSR cookie forwarding + vercel bypass + vercelBypassSecret config + Strapi ?json=true branch (wave 1)
- [ ] 129-04-PLAN.md — OAuth client surface: popup no-jwt, exchange routes, One Tap drop setToken (wave 2)
- [ ] 129-05-PLAN.md — verify-code/uploads/logout/middleware guards drop client token (wave 2)
