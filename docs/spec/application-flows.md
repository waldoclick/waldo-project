...
**Restore cron source** (documented in `docs/domain/reservation-system.md`, corrected 2026-07-08): the file is `apps/strapi/src/cron/ad-free-reservation-restore.cron.ts` (singular `cron/` directory, default-exported class `UserCronService`). Its `restoreFreeAds()` method restores **`AdReservation` (free ad-listing) slots only** — it does not touch `AdFeaturedReservation` records; there is no separate scheduled featured-reservation restore job (an earlier `featuredCron` was implemented and then reverted, per `.planning/STATE.md`'s decision log — the free-slot guarantee for featured reservations is not currently cron-enforced).
